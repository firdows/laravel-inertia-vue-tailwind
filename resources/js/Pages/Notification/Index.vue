    ...
    <section v-if="notifications.data.length" class="text-gray-700 dark:text-gray-400">
        <div v-for="notification in notifications.data" :key="notification.id"
            ...
            <div>
                ...
                <Link v-if="!notification.read_at" class="btn-outline text-xs font-medium uppercase" as="button"
                    :href="route('notification.seen', { notification: notification.id })" method="PUT">
                Mark as read
                </Link>
            </div>
        </div>
